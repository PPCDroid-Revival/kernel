...
void InitTo330Pointer( UCHAR ChipType ,PVB_DEVICE_INFO pVBInfo)
{
    ...
    if ( ChipType == XG27 )
    {
        ...
        temp = XGINew_GetReg1( pVBInfo->P3c4 , 0x3B ) & 0x88;
        if ( (temp ==0x80) || (temp ==0x08) )  /* SR3B[7][3]MAA15 MAA11 (Power on Trapping) */
        {   /*Z11m DDR*/
...
/* if ( pVBInfo->IF_DEF_ExpLink ) */
    ...
    if ( HwDeviceExtension->jChipType >= XG21 )
    {
      ...
      if ( temp & 0xA0 )
      {
        ...
        temp = 0;
        if ( ( HwDeviceExtension->jChipType == XG20 ) || ( HwDeviceExtension->jChipType == XG21 ) )
        {
           temp = HwDeviceExtension->b20A1ECOIDIs21;
        }
        if ( HwDeviceExtension->jChipType == XG27 )
        {
           temp = XGINew_GetReg1( pVBInfo->P3d4 , 0xCB ) & 0x80;
        }
        
        if ( !temp )
        {
            XGINew_SetRegAND( pVBInfo->P3c4 , 0x1E , ~0x80 ) ;
        XGI_UpdateXG21CRTC( ModeNo , pVBInfo , RefreshRateTableIndex) ;
        }
        else
        {
            XGINew_SetRegANDOR( pVBInfo->P3c4 , 0x1E , ~0x80 , 0x80 ) ;
        }
...
void XGI_GetVBInfo( USHORT ModeNo , USHORT ModeIdIndex , PXGI_HW_DEVICE_INFO HwDeviceExtension, PVB_DEVICE_INFO  pVBInfo )
{
    ...
    if ( pVBInfo->VBType & 0xFFFF )
    {
       ...
       if ( pVBInfo->IF_DEF_CH7007 == 1 )  /* [Billy] 07/05/03 */
       {
                tempbx = tempbx | SetCRT2ToSVIDEO ; /*can't use SetCRT2ToTV */
       }
...
void XGI_SenseCRT1( PVB_DEVICE_INFO pVBInfo )
{
    ...
    XGINew_SetReg1( pVBInfo->P3c4 , 0x1F , ( UCHAR ) SR1F ) ; /* Alan 2008/2/20 CRT DAC level is higer than normal case */
}
